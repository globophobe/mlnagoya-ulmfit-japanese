...
---
# A little while ago, I finished [Fast.ai](http://fast.ai) lesson four.
...
# A little while ago, "[An Investigation of Transfer Learning-Based Sentiment Analysis in Japanese](https://arxiv.org/abs/1905.09642)" was published on arXiv.org
## 少し前、ULMFiTと日本語自然言語処理に関する論文がarXiv.orgで公開されました。

---

# That paper was published by researchers at the company ExaWizards in Tokyo.
## 論文は東京のExaWizards株式会社の研究者が発表されました。
...
# Wikipedia data is used for the pretrained model.
## ウィキペディアデータは事前トレーニングモデルに使用されます。

</SplitRight>

---

# The datasets for fine-tuning were the [Japanese Rakuten product review binary and 5 class datasets](https://github.com/zhangxiangxiao/glyph)...
## 微調整のためのデータセットは、日本の楽天製品レビューデータセットで、
...
# There is a [ulmfit-multilingual](https://github.com/tsurushun/ulmfit-multilingual/tree/japanese) github fork for Japanese.
## githubには、日本語用のulmfit-multilingual gitフォークがあります。

---

# It is for slightly older version of Fast.ai
...
# There are 5 classes, and they are unbalanced.
## 5つのクラスがあり、クラスは不均衡です。
...
# Because I used Wikitext-2, not Wikitext-103, I decided to use 10,000 reviews from the Yahoo dataset.
## Wikitext-103ではなくって、Wikitext-2を使用したので、Yahooデータセットから10,000件のレビューを使用する事にしました。
...
# The ulmfit-multilingual repo uses SentencePiece for tokenization.
## トークン化のために、ulmfit-multilingualリポジトリはSentencePieceを使用します。

---

# So, I used it, but I was not satisfied.
## なので、それを利用しましたが、トークン化には満足していませんでした。
